...
function sectionTmdl(g) {
  ...
  else if (laOver.length) { tone = 'warn'; headline = `Diffuse load beyond the ΣLA on ${names(laOver)}`; sub = `The river carries ${fmtLoad(laOver.reduce((s, b) => s - b.laRemaining, 0))} more background and diffuse load than the allocation allows for. ${fmtVol(head.volume)} is left to licence at Standard A.`; }
  else { tone = 'ok'; headline = `Within allocation — ${binding} is binding`; sub = `${fmtVol(head.volume)} of new effluent could still be licensed at Standard A, about ${nf(headroomInPE(head.volume))} population equivalent.`; }
