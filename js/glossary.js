...
export const GLOSSARY = [
  ...
  },
  {
    ...
    terms: [
      ...
      ['What is around this station', 'Everything the map carries within a chosen distance of the station: the surveyed premises by kind, the water they can reach, and the next station downstream.',
        'The distance is straight-line. 500 m to 5 km.'],
      ['Shops and businesses', 'A live lookup from OpenStreetMap for what the pollution-source survey does not cover: shops, workshops, car washes, restaurants, clinics.',
        'A lookup, not a record. Nothing it finds is written into the licence register.'],
      ['Licence term', 'The dates a discharge licence was issued and runs out. The register counts down to the expiry and warns before it.',
        'Expired, expiring within the warning window, valid beyond it, or no date on record — which is not the same as expired.'],
      ['No expiry date', 'An entry the register can say nothing about: it cannot be counted down or warned before. Listed on the Licences tab with a button to fill the term in.',
        'Kept apart from expired, which is an allegation rather than a gap.'],
      ['Volume charge', 'Return water charged by volume: tiered between RM0.10 and RM0.20 per cubic metre a month, on the LUAS return water charge schedule.',
        'The two rates come from the source. The boundary between them, and whether the tiers apply per band or to the whole volume, are assumptions set on the page.'],
      ['Mass charge', 'Pollutant charged per kilogram, but only the mass above the standard limit, under the Licensing Regulations (Selangor) 2024, Fourth Schedule.',
        'BOD, COD, suspended solids, ammoniacal nitrogen and oil and grease. No rate per kilogram came with the source, so it starts at nought and no charge is invented.'],
      ['Oil and grease', 'Charged above 1 mg/L under Standard A and 10 mg/L under Standard B, and recorded on the licence only when someone has measured it.',
        'It has no INWQS ambient standard here, so it is charged without entering the TMDL.'],
      ['Renewal', 'Recording a new term without losing the old one: the term being replaced is pushed onto the entry’s history, and the new one written over the top.',
        'A continuous renewal starts the day after the previous term; one recorded after a lapse starts today. Renewals recorded lists them.'],
      ['Warning window', 'How much notice the register gives before a licence runs out: 30, 60 or 90 days.',
        'A month is enough to start a renewal; three months is enough to plan an inspection round. The count sits on the nav entry from any page.'],
  ...
  },
  {
    ...
    terms: [
      ...
      ['Loading capacity', 'Standard concentration × design flow × 0.024, with the flow in m³/h, in kg/day.',
      ...
      ['Current load', '12-month median concentration × design flow × 0.024, in kg/day.',
      ...
      ['Design flow', 'The low-flow condition a TMDL is written for (MAM7 / 7Q10), not the mean, in cubic metres an hour. Part of the record; 16,200 m³/h at Dengkil as an estimate, and every other station scaled from it by the channel length draining to it.',
