...
export const GLOSSARY = [
  ...
  },
  {
    ...
    terms: [
      ...
      ['Licence register pin', 'A ring on a premises, or a pin at a coordinate, for each entry in the licence register.',
        'Marks the worked examples and anything you add; estimated licences show only as the green fill.'],
    ],
  },
  {
    ...
    terms: [
      ['TMDL', 'Total Maximum Daily Load: the mass of a pollutant a reach can receive per day and still hold its class. TMDL = ΣWLA + ΣLA + MOS.',
        'The frame Phase 3 is built on: what the water can carry, what it carries now, what is left to licence.'],
      ['Loading capacity', 'Standard concentration × design flow × 86.4, in kg/day.',
        'The TMDL figure for one pollutant at the target class.'],
      ['WLA', 'Wasteload allocation: the share of the capacity given to licensed point sources.',
        'Summed from the licence register.'],
      ['LA', 'Load allocation: the share for diffuse and background sources with no permit — run-off, drains, land use.',
        'What is left of the river’s load once the licences are subtracted.'],
      ['MOS', 'Margin of safety, held back before anything is allocated; 10% here.',
        'Covers what the estimate does not know.'],
      ['Allocable', 'Capacity minus the margin of safety.',
        'The amount actually available to split between licensed and diffuse sources.'],
      ['Current load', '12-month median concentration × design flow × 86.4, in kg/day.',
        'What the river carries now at the selected station.'],
      ['Remaining', 'Allocable minus current load.',
        'Positive: room for more. Negative: the reach is over capacity and nothing further can be licensed.'],
      ['Diffuse load', 'Current load minus the licensed load.',
        'Background, run-off and unlicensed discharge that no permit accounts for.'],
      ['Design flow', 'The low-flow condition a TMDL is written for (MAM7 / 7Q10), not the mean; 4.5 m³/s at Dengkil, an estimate.',
      ...
      ['Remaining licensable volume', 'The remaining load divided by the standard concentration, in m³/day.',
